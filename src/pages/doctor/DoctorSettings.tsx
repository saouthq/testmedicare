...
import { User, Clock, Bell, Shield, MapPin, Phone, Mail, Save } from "lucide-react";
          ...
          <div className="space-y-6">
            <div className="rounded-xl border bg-card p-6 shadow-card">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div><Label>Prénom</Label><Input defaultValue="Sophie" className="mt-1" /></div>
                <div><Label>Nom</Label><Input defaultValue="Martin" className="mt-1" /></div>
                <div><Label>Email</Label><Input defaultValue="sophie.martin@mediconnect.fr" className="mt-1" /></div>
                <div><Label>Téléphone</Label><Input defaultValue="01 42 60 12 34" className="mt-1" /></div>
                <div className="sm:col-span-2"><Label>Adresse du cabinet</Label><Input defaultValue="12 rue de la Paix, 75002 Paris" className="mt-1" /></div>
              </div>
            </div>
            <div className="rounded-xl border bg-card p-6 shadow-card">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><Label>N° RPPS</Label><Input defaultValue="10101010101" className="mt-1" /></div>
                <div><Label>N° ADELI</Label><Input defaultValue="750000001" className="mt-1" /></div>
                <div><Label>Tarif consultation (€)</Label><Input defaultValue="25" className="mt-1" type="number" /></div>
                <div className="sm:col-span-2"><Label>Présentation</Label>
                  <textarea defaultValue="Médecin généraliste diplômée de la Faculté de Médecine de Paris." rows={3} className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <div><Label>Durée de consultation (min)</Label><Input defaultValue="30" className="mt-1" type="number" /></div>
                <div><Label>Langues parlées</Label><Input defaultValue="Français, Anglais" className="mt-1" /></div>
              </div>
            </div>
          ...
          <div className="rounded-xl border bg-card p-6 shadow-card">
            ...
            <div className="space-y-4">
              {weekDays.map(day => {
                ...
                return (
                  <div key={day} className={`rounded-lg border p-4 transition-colors ${dayAvail.active ? "bg-card" : "bg-muted/50"}`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <label className="flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          <span className={`text-sm font-medium ${dayAvail.active ? "text-foreground" : "text-muted-foreground"}`}>{day}</span>
                        </label>
                      </div>
                      {!dayAvail.active && <span className="text-xs text-muted-foreground">Fermé</span>}
                    </div>
                    {dayAvail.active && (
                      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
                        ...
                        <div>
                          ...
                          <Input
                            type="time"
                            value={dayAvail.breakStart}
                            onChange={e => setAvailability(prev => ({ ...prev, [day]: { ...prev[day], breakStart: e.target.value } }))}
                            className="mt-1"
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            type="time"
                            value={dayAvail.breakEnd}
                            onChange={e => setAvailability(prev => ({ ...prev, [day]: { ...prev[day], breakEnd: e.target.value } }))}
                            className="mt-1"
                          />
                        </div>
                      </div>
                ...
                <div key={i} className="flex items-center justify-between py-3 border-b last:border-0">
                  ...
                  <div className="flex gap-3">
                    <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <input type="checkbox" defaultChecked className="rounded border-input" />Email
                    </label>
                    <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <input type="checkbox" defaultChecked className="rounded border-input" />SMS
                    </label>
                  </div>
                </div>
              ))}
