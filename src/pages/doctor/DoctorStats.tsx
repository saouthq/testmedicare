...
import { TrendingUp, Users, Calendar, Euro, Clock, Activity, ChevronDown } from "lucide-react";
...
const stats = [
  ...
  { label: "CA mensuel", value: "3 150 €", change: "+8%", icon: Euro, color: "text-warning" },
          ...
          {stats.map(s => (
            <div key={s.label} className="rounded-xl border bg-card p-5 shadow-card">
              ...
              <div className="flex items-end gap-2 mt-2">
                ...
                <span className={`text-xs font-medium ${s.change.startsWith("+") ? "text-accent" : s.change.startsWith("-") ? "text-destructive" : "text-muted-foreground"}`}>
                  {s.change}
                </span>
              </div>
            </div>
          ...
          <div className="rounded-xl border bg-card p-5 shadow-card lg:col-span-2">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={weeklyRevenue}>
                ...
                <YAxis tick={{ fontSize: 12, fill: "hsl(215, 15%, 50%)" }} tickFormatter={v => `${v}€`} />
