...
import { Calendar, ChevronLeft, ChevronRight, MapPin, Star, Clock, CheckCircle2, ArrowLeft, User } from "lucide-react";
...
const doctor = {
  ...
  presentation: "Médecin généraliste diplômée de la Faculté de Médecine de Paris. Spécialisée dans le suivi des maladies chroniques et la médecine préventive. Consultation sur rendez-vous uniquement.",
};
  ...
  return (
    <DashboardLayout role="patient" title="Prendre rendez-vous">
      <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                <span className="flex items-center gap-1">
                  <MapPin className="h-3.5 w-3.5" />{doctor.address}
                </span>
                <span className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-4 mt-3">
                <span className="text-sm font-medium text-foreground">Tarif : {doctor.consultationPrice}€</span>
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                <div className={`h-8 w-8 rounded-full flex items-center justify-center text-sm font-medium ${
                  stepIndex < currentIndex ? "bg-accent text-accent-foreground" :
                  stepIndex === currentIndex ? "gradient-primary text-primary-foreground" :
                  "bg-muted text-muted-foreground"
                }`}>
                  {stepIndex < currentIndex ? <CheckCircle2 className="h-4 w-4" /> : stepIndex + 1}
                </div>
                <span className={`text-sm hidden sm:inline ${stepIndex === currentIndex ? "font-medium text-foreground" : "text-muted-foreground"}`}>
                  {label}
                </span>
        ...
        {step === "slot" && (
          <div className="rounded-xl border bg-card p-6 shadow-card">
            <div className="flex items-center justify-between mb-6">
              <button onClick={() => setStep("motif")} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                    ...
                    {(availableSlots[day] || []).map((slot) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full rounded-lg border py-2 text-sm transition-all ${
                            isSelected
        ...
        {step === "confirm" && (
          <div className="rounded-xl border bg-card p-6 shadow-card">
            <button onClick={() => setStep("slot")} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6">
              <ArrowLeft className="h-4 w-4" />Retour
            </button>
            ...
            <div className="space-y-4">
              <div className="rounded-lg bg-muted/50 p-4 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">{doctor.name} — {doctor.specialty}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">{selectedDay} Février 2026 à {selectedSlot}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">{selectedMotif}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
              ...
              <div className="rounded-lg border border-warning/30 bg-warning/5 p-4">
                <p className="text-sm text-foreground">
                  <strong>Important :</strong> Merci de vous présenter 10 minutes avant l'heure de votre rendez-vous avec votre carte vitale et carte de mutuelle.
                </p>
              </div>

              <div className="flex items-center gap-2">
                <input type="checkbox" id="sms" className="rounded border-input" defaultChecked />
                <label htmlFor="sms" className="text-sm text-muted-foreground">Recevoir un rappel par SMS</label>
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="email" className="rounded border-input" defaultChecked />
                <label htmlFor="email" className="text-sm text-muted-foreground">Recevoir un rappel par email</label>
              </div>
            </div>

            <div className="mt-6 flex justify-end gap-3">
              <Button variant="outline" onClick={() => setStep("slot")}>Modifier</Button>
              <Button className="gradient-primary text-primary-foreground shadow-primary-glow" onClick={handleConfirm}>
        ...
        {step === "done" && (
          <div className="rounded-xl border bg-card p-8 shadow-card text-center">
            ...
            <p className="mt-2 text-muted-foreground">
              Votre rendez-vous avec {doctor.name} est confirmé pour le {selectedDay} Février 2026 à {selectedSlot}.
            </p>
            <p className="mt-1 text-sm text-muted-foreground">Un email de confirmation vous a été envoyé.</p>
            <div className="mt-6 flex justify-center gap-3">
              <Link to="/dashboard/patient/appointments">
                <Button variant="outline">Voir mes RDV</Button>
              </Link>
              <Link to="/dashboard/patient">
                <Button className="gradient-primary text-primary-foreground">Retour au tableau de bord</Button>
              </Link>
            </div>
          </div>
        )}
