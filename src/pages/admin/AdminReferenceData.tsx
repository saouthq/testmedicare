...
type RefTab = "specialties" | "cities" | "languages" | "motifs";
...
const tabLabels: Record<RefTab, string> = { specialties: "Spécialités", cities: "Villes", languages: "Langues", motifs: "Motifs" };
      ...
      <div className="space-y-6 max-w-3xl">
        <div className="flex gap-1 rounded-lg border bg-card p-0.5 w-fit">
          {(Object.entries(tabLabels) as [RefTab, string][]).map(([k, v]) => (
            <button key={k} onClick={() => setTab(k)} className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${tab === k ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>{v}</button>
          ))}
        </div>
