/**
 * Admin Email & SMS Configuration — Gateway settings, delivery stats, test sends
...
import {
  ...
  Settings, Save, TestTube, Eye, BarChart3, RefreshCw, Zap,
} from "lucide-react";
...
const recentSends = [
  ...
  { id: "s-7", channel: "sms", to: "+216 71 111 222", template: "Résultat disponible", status: "delivered", time: "09:00" },
  { id: "s-8", channel: "push", to: "Fatma T.", template: "Rappel RDV", status: "delivered", time: "08:55" },
];
...
const AdminEmailConfig = () => {
  ...
  // Email config
  const [smtpHost, setSmtpHost] = useState("smtp.medicare.tn");
  const [smtpPort, setSmtpPort] = useState("587");
  const [smtpUser, setSmtpUser] = useState("noreply@medicare.tn");
  const [smtpTls, setSmtpTls] = useState(true);
  const [fromName, setFromName] = useState("Medicare.tn");
  const [fromEmail, setFromEmail] = useState("noreply@medicare.tn");
  const [replyTo, setReplyTo] = useState("support@medicare.tn");

  // SMS config
  const [smsProvider, setSmsProvider] = useState("tunisie_telecom");
  const [smsApiKey, setSmsApiKey] = useState("••••••••••••••••");
  const [smsSenderId, setSmsSenderId] = useState("MEDICARE");
      ...
      <div className="space-y-6">
        <Tabs defaultValue="stats">
                ...
                <TableBody>
                  {recentSends.map(s => {
                    ...
                    return (
                      <TableRow key={s.id}>
                        <TableCell>
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium uppercase ${channelColors[s.channel]}`}>{s.channel}</span>
                        </TableCell>
                        ...
                        <TableCell>
                          <span className={`flex items-center gap-1 text-xs font-medium ${statusColors[s.status]}`}>
                            <StatusIcon className="h-3 w-3" />{statusLabels[s.status]}
                          </span>
                        </TableCell>
          ...
          <TabsContent value="email" className="space-y-6 max-w-2xl">
            ...
            <div className="flex items-center justify-between">
              <Button variant="outline" size="sm" onClick={handleTestEmail}>
                <TestTube className="h-3.5 w-3.5 mr-1" />Envoyer un email de test
              </Button>
          ...
          <TabsContent value="sms" className="space-y-6 max-w-2xl">
            <div className="rounded-xl border bg-card p-6 shadow-card space-y-5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Label className="text-xs text-muted-foreground">Service actif</Label>
            ...
            <div className="rounded-xl border bg-card p-6 shadow-card space-y-4">
              <h3 className="font-semibold text-foreground">Quotas & Coûts</h3>
              <div className="grid gap-3 grid-cols-2">
                <div className="rounded-lg border p-3 text-center">
                  <p className="text-lg font-bold text-foreground">12,450</p>
                  <p className="text-[10px] text-muted-foreground">SMS envoyés ce mois</p>
                </div>
                <div className="rounded-lg border p-3 text-center">
                  <p className="text-lg font-bold text-foreground">50,000</p>
                  <p className="text-[10px] text-muted-foreground">Quota mensuel</p>
                </div>
                <div className="rounded-lg border p-3 text-center">
                  <p className="text-lg font-bold text-primary">0.035 DT</p>
                  <p className="text-[10px] text-muted-foreground">Coût / SMS</p>
                </div>
                <div className="rounded-lg border p-3 text-center">
                  <p className="text-lg font-bold text-foreground">435.75 DT</p>
                  <p className="text-[10px] text-muted-foreground">Coût total ce mois</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <Button variant="outline" size="sm" onClick={handleTestSms}>
                <TestTube className="h-3.5 w-3.5 mr-1" />Envoyer un SMS de test
              </Button>
              <Button className="gradient-primary text-primary-foreground" onClick={handleSave}>
                <Save className="h-3.5 w-3.5 mr-1" />Enregistrer
              </Button>
          ...
          <TabsContent value="push" className="space-y-6 max-w-2xl">
            ...
            <div className="rounded-xl border bg-card p-6 shadow-card space-y-4">
              <h3 className="font-semibold text-foreground">Statistiques Push</h3>
              <div className="grid gap-3 grid-cols-3">
                <div className="rounded-lg border p-3 text-center">
                  <p className="text-lg font-bold text-foreground">4,490</p>
