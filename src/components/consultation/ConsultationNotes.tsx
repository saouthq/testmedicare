/**
 * ConsultationNotes — Zone centrale, adaptée par spécialité
 * Les sections de notes changent dynamiquement selon la spécialité du médecin.
 */
...
function NoteSection({
  ...
  const accentMap: Record<string, { bar: string; bg: string; text: string; badge: string }> = {
    blue: { bar: "bg-blue-500", bg: "bg-blue-50/60 dark:bg-blue-950/30", text: "text-blue-600 dark:text-blue-400", badge: "bg-blue-100 dark:bg-blue-900/50" },
    amber: { bar: "bg-amber-400", bg: "bg-amber-50/60 dark:bg-amber-950/30", text: "text-amber-600 dark:text-amber-400", badge: "bg-amber-100 dark:bg-amber-900/50" },
    teal: { bar: "bg-teal-500", bg: "bg-teal-50/60 dark:bg-teal-950/30", text: "text-teal-600 dark:text-teal-400", badge: "bg-teal-100 dark:bg-teal-900/50" },
    violet: { bar: "bg-violet-500", bg: "bg-violet-50/60 dark:bg-violet-950/30", text: "text-violet-600 dark:text-violet-400", badge: "bg-violet-100 dark:bg-violet-900/50" },
    emerald: { bar: "bg-emerald-500", bg: "bg-emerald-50/60 dark:bg-emerald-950/30", text: "text-emerald-600 dark:text-emerald-400", badge: "bg-emerald-100 dark:bg-emerald-900/50" },
  };
...
export function ConsultationNotes() {
  ...
  // Map note section IDs to context getters/setters
  const noteFieldMap: Record<string, { value: string; onChange: (v: string) => void }> = {
    motif: { value: ctx.motif, onChange: ctx.setMotif },
    symptoms: { value: ctx.symptoms, onChange: ctx.setSymptoms },
    examination: { value: ctx.examination, onChange: ctx.setExamination },
    diagnosis: { value: ctx.diagnosis, onChange: ctx.setDiagnosis },
    conclusion: { value: ctx.conclusion, onChange: ctx.setConclusion },
  };

  const sections = config.noteSections;

  // Split: first section alone, middle sections, last two side-by-side
  const firstSection = sections[0];
  const middleSections = sections.slice(1, -2);
    ...
    <main className="space-y-3 min-w-0">
      ...
      <div className="rounded-2xl border bg-card shadow-sm p-3">
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <button
            onClick={() => { setTplName(""); setSaveOpen(true); }}
          ...
          <div className="flex flex-wrap gap-2">
            {ctx.templates.map((t) => (
              <div key={t.key} className="group relative flex items-center">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); ctx.deleteTemplate(t.key); toast({ title: "Template supprimé", description: t.label }); }}
      ...
      {/* ── Dynamic note sections from specialty config ── */}
      {firstSection && (
        <NoteSection
          id="notes"
          label={firstSection.label}
          hint={firstSection.hint}
          accent={firstSection.accent}
          value={noteFieldMap[firstSection.id]?.value ?? ""}
          onChange={noteFieldMap[firstSection.id]?.onChange ?? (() => {})}
          placeholder={firstSection.placeholder}
          singleLine={firstSection.singleLine}
      ...
      {middleSections.map((s) => (
        <NoteSection
          ...
          hint={s.hint}
          accent={s.accent}
          value={noteFieldMap[s.id]?.value ?? ""}
          onChange={noteFieldMap[s.id]?.onChange ?? (() => {})}
      ...
      {lastTwo.length === 1 && (
        <NoteSection
          label={lastTwo[0].label}
          hint={lastTwo[0].hint}
          accent={lastTwo[0].accent}
          value={noteFieldMap[lastTwo[0].id]?.value ?? ""}
          onChange={noteFieldMap[lastTwo[0].id]?.onChange ?? (() => {})}
          placeholder={lastTwo[0].placeholder}
          rows={lastTwo[0].rows}
          required={lastTwo[0].required}
        />
      )}
      ...
      <Dialog open={saveOpen} onOpenChange={setSaveOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            Les notes actuelles seront sauvegardées pour vos prochaines consultations similaires.
          </p>
          <div className="space-y-2 py-2">
            ...
            <Input
              ...
              placeholder={`Ex : ${config.motifs?.[0] || "Template personnalisé"}…`}
